...
export default function MediaCard({ recommendation }: { recommendation: ScoredRecommendation }) {
  const { url, name, type, size, tags, uploadDate, score, reason } = recommendation;
  const [imageFailed, setImageFailed] = useState(false);
  const showImage = type === 'image' && !imageFailed;
      ...
      <div className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <h4 className="font-medium text-on-surface truncate">{name}</h4>
        ...
        <p className="text-xs text-text-muted/60 mt-1">Uploaded {formatDate(uploadDate)}</p>
      </div>
    </div>
  );
}
